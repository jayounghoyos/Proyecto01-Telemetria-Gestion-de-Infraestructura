...
int main(void) {
    ...
    if (pthread_create(&udp_thread, NULL, udp_telemetry_server, &udp_fd) ||
        pthread_create(&tcp_thread, NULL, tcp_command_server, &tcp_fd) ||
        pthread_create(&http_thread, NULL, http_status_server, &http_fd)) {
        fprintf(stderr, "Cannot start services\n"); return EXIT_FAILURE;
    }
    printf("[main] TELEP/2.0 server ready (tcp %d, udp %d, http %d)\n", TCP_PORT, UDP_PORT, HTTP_PORT);
