...
int registry_record_telemetry(const char *node_id, long sequence, const Measurement *values, int count) {
    ...
    for (int i = 0; i < count; i++) {
        double limit = alert_threshold(values[i].name);
        if (limit > 0 && values[i].value > limit)
            add_alert(node_id, values[i].name, "HIGH", values[i].value);
        else if (strcmp(values[i].name, "STATUS") == 0 && values[i].value == 0.0)
